...
function CleveRoids.IsValidTarget(target, help)
    local unitToCheck = target

    -- If the conditional is for @mouseover, use the globally cached unit.
    if target == "mouseover" then
        unitToCheck = CleveRoids.mouseoverUnit
        -- If the cached unit is nil, it means there is no valid mouseover target, so the condition fails instantly.
        if not unitToCheck then
            return false
        end
    end

    -- Perform the final, simple check on the determined unit.
    if not UnitExists(unitToCheck) or not CleveRoids.CheckHelp(unitToCheck, help) then
        return false
    end
