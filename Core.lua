...
-- Attempts to attack a unit by a set of conditionals
-- msg: The raw message intercepted from a /startattack command
function CleveRoids.DoStartAttack(msg)
    local handled = false

    -- This action is for conditional calls. It assumes a target has already been
    -- validated and set by the DoWithConditionals function.
    local conditional_action = function()
    ...
    -- This action is for non-conditional calls (/startattack with no arguments).
    -- It preserves the original behavior of finding a new target if the current one is invalid.
    local non_conditional_action = function()
         if not UnitExists("target") or UnitIsDead("target") or not UnitCanAttack("player", "target") then
            TargetNearestEnemy()
         end
         -- After potentially finding a target, execute the core attack logic.
         conditional_action()
    end

    -- DoWithConditionals will call the 'hook' (non_conditional_action) if there are no conditionals,
    -- and the 'action' (conditional_action) if conditionals are present and pass their checks.
    for k, v in pairs(CleveRoids.splitStringIgnoringQuotes(msg)) do
        if CleveRoids.DoWithConditionals(v, non_conditional_action, CleveRoids.FixEmptyTarget, true, conditional_action) then
            handled = true
            break
        end
    end
...
-- Attempts to stop an attack based on a set of conditionals
-- msg: The raw message intercepted from a /stopattack command
function CleveRoids.DoStopAttack(msg)
    local handled = false

    -- For /stopattack, the action is the same with or without conditionals.
    -- If you are auto-attacking, it will be toggled off.
    ...
    -- DoWithConditionals will call the 'action' function if the message has no conditionals (via the hook),
    -- or if it has conditionals that pass their checks.
    for k, v in pairs(CleveRoids.splitStringIgnoringQuotes(msg)) do
...
-- Attempts to stop casting a spell based on a set of conditionals
-- msg: The raw message intercepted from a /stopcasting command
function CleveRoids.DoStopCasting(msg)
    local handled = false

    -- The action is always to call the SpellStopCasting() API function.
    local action = function()
        SpellStopCasting()
    end
